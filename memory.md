...
| Domain | `hustlr.online` | `app.constants.ts` |
...
| Support email | `support@hustlr.online` | `app.constants.ts` |
...
The auto-generated `frontend/AGENTS.md` warns: *"This is NOT the Next.js you know — APIs, conventions, and file structure may all differ from your training data."*
...
| 6 | Cookie-backed JWT auth | Platform `hustlr_session` (Domain=.hustlr.online) + host-only buyer `hustlr_buyer_session`. No localStorage for session tokens. Coarse Edge middleware protection with authoritative backend authorization. Direct browser-to-backend production communication. | 2026-08-20 |
...
| 8 | Deployment & infrastructure | Primary on Vercel (Edge middleware, `*.hustlr.online` wildcard SSL, branch previews). Backend on independent Express/Node deployment (`api.hustlr.online`). Standalone Docker portability preserved via `output: 'standalone'`. External DNS/domain APIs isolated behind `DomainProvider` abstraction. | 2026-08-20 |
