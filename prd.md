...
Hustlr is a **multi-tenant e-commerce SaaS platform** for African merchants. Sellers register on the main platform (`hustlr.shop`), configure a branded storefront, and receive a subdomain (e.g. `musa-store.hustlr.shop`). Buyers shop on individual seller storefronts. Payments flow through Paystack using an escrow model.
...
| Support email | `support@hustlr.shop`                                                                                                                                     |
...
| ---------- | -------------------------------------------------------------------- | ------------------------------------------------ |
| **Admin**  | Platform operators managing Hustlr itself                            | Main platform (`hustlr.shop`)                    |
| **Seller** | Store owners who register, set up stores, manage products and orders | Main platform (`hustlr.shop`)                    |
| **Buyer**  | Customers shopping on a seller's storefront; scoped per-store        | Seller subdomain (e.g. `musa-store.hustlr.shop`) |
...
6. **Subscriptions & Going Live (`/(dashboard)/dashboard/billing`)**: Plan selection (Free, Pro @ ₦15k/mo, Pro+ @ ₦35k/mo) → Paystack payment → Store live celebration on `{slug}.hustlr.shop`.
...
1. **Subdomain Routing**: Middleware rewrite `*.hustlr.shop` → `/store/[slug]` with `*.lvh.me` dev testing.
2. **Route Group Organization**: 5 isolated route groups `(marketing)`, `(auth)`, `(dashboard)`, `(storefront)`, `(admin)` with dedicated layouts.
3. **Authentication Architecture**: Cookie-backed JWT (`hustlr_session` on `.hustlr.shop` + host-only `hustlr_buyer_session`), no localStorage.
